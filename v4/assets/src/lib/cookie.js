define(function(require, exports, module) {
	var cookie = {
		...
		set: function(key, value, ttl, path, domain, secure) {
			data = [key + "=" + escape(value), "path=" + ((!path || path == "") ? "/" : path), "domain=" + ((!domain || domain == "") ? window.location.hostname : domain)];
			if (ttl) {
				data.push("expires=" + secondsToExpireDate(ttl))
			}
			if (secure) {
				data.push("secure")
			}
			return document.cookie = data.join("; ")
		},
